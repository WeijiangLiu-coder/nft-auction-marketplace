...
contract AuctionFactoryV2 is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getAuctionStatus(address _auctionAddress) public view returns (Auction.AuctionStatus) {
        return Auction(payable(_auctionAddress)).status();
    }
    function endAuction(address _auctionAddress) public {
        Auction(payable(_auctionAddress)).endAuction();
    }
    // 获取所有拍卖地址
