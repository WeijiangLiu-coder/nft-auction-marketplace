    ...
    AccessControlUpgradeable
{
    ...
    struct Pool {
        ...
        // 每个stToken的MetaNode奖励
        uint256 accMetaNodePerST;
        // 抵押的stoken数量
        uint256 stTokenAmount;
        // 最小抵押数量
        uint256 minDepositAmount;
        // 质押解锁区块数（发起提取后，需等待该数量区块才能实际提取）
        uint256 unstakeLockedBlocks;
    }
    //提取请求结构体：存储用户发起的质押提取请求
    ...
    function addPool(address _stTokenAddress, uint256 _poolWeight, uint256 _minDepositAmount, uint256 _unstakeLockedBlocks,  bool _withUpdate) public onlyRole(ADMIN_ROLE) {
        ...
        // 计算新池的初始奖励区块：若当前区块已过开始区块，用当前区块；否则用开始区块
    ...
    /**计算从 _from 到 _to 区块区间内，总共有多少 MetaNode 代币奖励
    ...
    /** 查询用户在指定池的当前待领取MetaNode奖励（用当前区块号）
