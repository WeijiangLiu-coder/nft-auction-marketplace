...
contract FLOKI is IERC20, IGovernanceToken, Ownable {
    ...
    /// @notice Registry of user delegates for governance.用于治理功能的用户委托代表注册表，每个地址把投票权委托给了谁
    mapping(address => address) public delegates;

    /// @notice Registry of nonces for vote delegation.防止 “签名重放攻击”
    mapping(address => uint256) public nonces;

    /// @notice Registry of the number of balance checkpoints an account has.存某个地址有多少条历史记录
    mapping(address => uint32) public numCheckpoints;

    /// @notice Registry of balance checkpoints per account.存每条记录的具体内容，包括区块号和投票
    ...
    /**
     * @notice 代币的总供应量 Get the maximum number of tokens.
     ...
     * own address.如果想 “自己投票”，必须把 delegatee 设为自己的地址（比如你调用 delegate(自己的地址)），否则没有投票权。
    ...
    /**
     * @notice Move delegates from one address to another.将代表从一个地址移到另一个地址。
     * @param from Representative to move delegates from.  从哪个代表移除
     * @param to Representative to move delegates to.  移动到哪个代表
     * @param amount Number of delegates to move. 移动多少个代表
     */
